...
func create_player() -> void: # player object has already been created, so just register it
	...
	Debug.log_debug("Created player: %s" % blueprint.player.profile.profile_name)
...
func create_countries() -> void:
	...
	Debug.log_debug("Created %d countries" % countries.datastore.size())
...
func create_cities() -> void:
	...
	Debug.log_debug("Created %d cities" % cities.datastore.size())
