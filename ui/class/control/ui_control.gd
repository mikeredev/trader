...
func fade_background(p_toggled_on: bool) -> void: # avoid changing color or handle tween interruptions better
	if get_node_or_null("%Background"):
		...
		if p_toggled_on:
			background_tween = System.manage.scene.create_tween(background, "modulate:a", 0.5, BG_FADE_DURATION)
			background.mouse_filter = Control.MOUSE_FILTER_STOP
		else:
			background_tween = System.manage.scene.create_tween(background, "modulate:a", 0.0, BG_FADE_DURATION)
			background.mouse_filter = Control.MOUSE_FILTER_IGNORE
