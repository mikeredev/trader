...
func build() -> void:
	...
	Debug.log_debug("Created: %d categories, %d markets, %d resources" % [
		Service.trade_manager.category_datastore.size(),
		Service.trade_manager.market_datastore.size(),
...
func _create_resource(resource_id: StringName, p_metadata: Dictionary) -> void:
	...
	# register for lookup
	Service.trade_manager.resource_datastore[resource_id] = resource
...
func _get_or_create_market(p_market_id: StringName) -> TradeMarket:
	...
	if Service.trade_manager.get_market(p_market_id):
		market = Service.trade_manager.get_market(p_market_id)
	else:
		# create object
		...
		# register for lookup
		Service.trade_manager.market_datastore[p_market_id] = market
...
func _get_or_create_category(p_category_id: StringName) -> TradeCategory: # lazy initialize category if needed
	...
	if Service.trade_manager.get_category(p_category_id):
		category = Service.trade_manager.get_category(p_category_id)
	else:
		# create object
		...
		# register for lookup
		Service.trade_manager.category_datastore[p_category_id] = category
